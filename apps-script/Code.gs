...
const SPREADSHEET_ID = '13opRk7UseFFHSKmj5VQ64uwYJrHWlMvT333wpawvs7g';
const TZ = 'America/Argentina/Buenos_Aires';
const URL_TABLERO = 'https://agustinalvarezspj.github.io/tablero-marazul/';
...
const NOMBRES = {
  agustin: 'Agustín', miriam: 'Miriam', martin: 'Martín',
  celeste: 'Celeste', crispin: 'Crispín', selva: 'Selva',
};
const AREAS_NOMBRE = {
  ventas: '💼 Ventas', compras: '🛒 Compras', mant: '🔧 Mantenimiento',
  house: '🛏 Housekeeping', tesoreria: '💰 Tesorería',
  marketing: '📣 Marketing', rrhh: '👥 RRHH',
};
  ...
  Object.keys(EMAILS).forEach(pid => {
    ...
    const html =
      '<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;background:#fefcf9;border:1px solid #ece6d8;border-radius:12px;overflow:hidden">' +
        '<div style="background:#0D2B5E;padding:18px 22px">' +
          '<div style="color:white;font-size:17px;font-weight:bold">🌊 Mar Azul Suites — Tablero Operativo</div>' +
          '<div style="color:#9fc3e8;font-size:13px;margin-top:3px">Hola ' + (NOMBRES[pid] || pid) + ', estas son tus tareas pendientes de hoy ' + hoyLindo + '</div>' +
        '</div>' +
        ...
        '<div style="padding:16px 22px;text-align:center">' +
          '<a href="' + URL_TABLERO + '" style="background:#0D2B5E;color:white;text-decoration:none;font-size:13px;padding:10px 22px;border-radius:8px;display:inline-block">Abrir el tablero</a>' +
        '</div>' +
      '</div>';
